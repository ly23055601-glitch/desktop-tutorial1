#!/usr/bin/env python3
"""Audit DJI Pocket comments for structure, repetition, and LH5–LH13 evidence."""
...
LH13_RULE_VERSION = "2026-09-05-LH13"
LH12_PLUS_RULE_VERSIONS = {LH12_RULE_VERSION, LH13_RULE_VERSION}
LH11_PLUS_RULE_VERSIONS = {LH11_RULE_VERSION, *LH12_PLUS_RULE_VERSIONS}
...
def load_seed_state(
    ...
    if state_path is None:
        add_seed_diag(
            ...
            policy,
            "SEED_STATE_MISSING",
            "LH5/LH6/LH7/LH8/LH9/LH10/LH11/LH12/LH13 种草检查缺少 --state；未执行逐链接证据映射",
        )
        return {}, {}
    ...
    for block in blocks:
        ...
        for allocation in allocations:
            ...
            material = allocation.get("user_material")
            if isinstance(material, dict):
                copied["_resolved_user_material_path"] = resolve_state_draft_path(material.get("path"), state_path)
...
def load_claim_registry(
    ...
    *, lh13: bool = False,
    ...
    try:
        ...
        return {}
    if lh13:
        extension = registry.get("lh13", {})
        if not isinstance(extension, dict) or not isinstance(extension.get("claims", {}), dict) or not isinstance(registry.get("claims"), dict):
            add_seed_diag(diagnostics, policy, "CLAIM_REGISTRY_INVALID", "lh13 claims 扩展必须为对象", file=registry_path, hard=True)
            return {}
        registry["claims"] = {**registry.get("claims", {}), **extension.get("claims", {})}
        if "model_ids" in extension:
            registry["model_ids"] = extension["model_ids"]
    ...
    registry_supported_models = LH7_CONTEXT_MODELS if lh13 else VALID_MODELS
    if registry_models is None or set(registry_models) != registry_supported_models:
        add_seed_diag(
            diagnostics,
            policy,
            "CLAIM_REGISTRY_INVALID",
            ...
            file=registry_path,
            hard=True,
            metrics={"actual": registry_models or [], "expected": sorted(registry_supported_models)},
        )
    ...
    if not isinstance(known_false_assertions, list):
        ...
        for index, assertion in enumerate(known_false_assertions):
            ...
            if not models or any(model not in registry_supported_models - {"unknown"} for model in models):
    ...
    for claim_id, claim in claims.items():
        ...
        if lh13 and "lh13_assertion_patterns" in claim:
            patterns = string_list(claim.get("lh13_assertion_patterns"))
            if not patterns:
                claim_schema_issues.append(f"{claim_id}:lh13_assertion_patterns")
            else:
        ...
        if not sources or any(
            not (lh13_official_source_key(source) if lh13 else normalized_source_url(source).startswith(("store.dji.com/", "www.dji.com/", "dji.com/")))
...
def audit_lh9_dialogues(sections: list[Section], contexts: dict[str, list[dict[str, Any]]], state: dict[str, Any], diagnostics: list[dict]) -> dict[str, Any]:
    ...
    checked = 0
    lh12 = state.get("rule_version") in LH12_PLUS_RULE_VERSIONS
        ...
        for group in section.groups:
            ...
            for field in ("dominant_expression", "product_connection"):
                if (field == "product_connection" and state.get("rule_version") in LH11_PLUS_RULE_VERSIONS
                        and lh11_visual_only_group(group, group_context, lh12=state.get("rule_version") in LH12_PLUS_RULE_VERSIONS, lh13=state.get("rule_version") == LH13_RULE_VERSION)):
                    continue
...
# LH13 adds a small declarative fact gate. It does not infer complete product
# truth from natural language; review must register new facts before delivery.
LH13_HARDWARE_TERM_RE = re.compile(
    LH8_HARDWARE_TERM_RE.pattern + r"|实体键|变焦键|按键|旋转|转镜头|换远近|戳屏幕|\d+\s*倍(?:镜头|中焦)|重量|配色|OLED", re.IGNORECASE)
LH13_CAPABILITY_ASSERTION_RE = re.compile(
    LH8_CAPABILITY_ASSERTION_RE.pattern + r"|不能|不支持|(?:能|可以|会)(?:自己|自动)?(?:旋转|转镜头)|"
    r"(?:自己|自动)(?:旋转|转镜头)|\d+\s*倍(?:镜头|中焦)|(?:实体)?变焦键|"
    r"(?:按一下|单击|双击|长按).{0,12}(?:换|切|变焦|放大)|不用戳屏幕", re.IGNORECASE)


def lh13_true_question(text: str) -> bool:
    """Question status is local to a clause; rhetorical certainty is factual."""
    return bool(QUESTION_RE.search(text) and not re.search(
        r"难道|难不成|不就是|不正是|岂不是|肯定|一定|当然|明明|显然|毫无疑问", text))


def lh13_official_source_key(value: Any) -> str:
    """Exact DJI host plus path; Help article identity includes customId.

    Tracking, storefront variant and locale query parameters are presentation
    details. A Help article's ID is content identity and must never be dropped.
    """
    try:
        parts = urlsplit(str(value).strip())
        if (parts.scheme.lower() != "https" or parts.hostname not in
                {"store.dji.com", "www.dji.com", "dji.com", "repair.dji.com"}
                or parts.username or parts.password or parts.port not in {None, 443}):
            return ""
        path = re.sub(r"/+", "/", parts.path or "/").rstrip("/") or "/"
        result = parts.hostname + path.lower()
        if parts.hostname == "repair.dji.com":
            ids = parse_qs(parts.query).get("customId", [])
            if path.lower() != "/help/content" or len(ids) != 1 or not re.fullmatch(r"[A-Za-z0-9_-]+", ids[0]):
                return ""
            result += "?customId=" + ids[0]
        return result
    except (ValueError, TypeError):
        return ""


def lh13_claim_matches(text: str, claim: dict[str, Any]) -> bool:
    """Required semantic fingerprints, deliberately limited and inspectable."""
    patterns = claim.get("lh13_assertion_patterns")
    if patterns:
        return any(re.search(pattern, text, re.IGNORECASE) for pattern in patterns)
    return normalized_contains(text, string_list(claim.get("text_terms", [])) or [])


def audit_lh13_product_facts(group: Group, context: dict[str, Any], claims: dict[str, Any],
                            assertions: list[str], issue: Any) -> tuple[list[str], set[str]]:
    """Validate quote-bound official facts independently of the group's voice.

    A valid record covers only matching assertions inside its quoted span. It
    never changes M/F/P, supplies footage provenance, or licenses causality.
    """
    records = context.get("product_facts", [])
    if not isinstance(records, list):
        issue("PRODUCT_FACTS_INVALID", "LH13 product_facts 必须是数组，纯感受或真实疑问可为空", hard=True)
        return assertions, set()
    valid_records: list[tuple[str, dict[str, Any], str]] = []
    seen: set[tuple[str, str]] = set()
    bodies = [item.text for item in [group.main, *group.replies]]
    for record in records:
        if not isinstance(record, dict):
            issue("PRODUCT_FACT_INVALID", "product_facts 每项必须为对象", hard=True)
            continue
        claim_id = record.get("claim_id")
        claim = claims.get(claim_id) if isinstance(claim_id, str) else None
        if not isinstance(claim, dict):
            issue("PRODUCT_FACT_UNKNOWN", "新产品事实须先登记 claim_id 和官方来源，不能以低S级或P0忽略", hard=True, metrics={"claim_id": claim_id})
            continue
        quote = record.get("exact_copy_quote")
        valid = True
        def fail(stem: str, message: str, **metrics: Any) -> None:
            nonlocal valid
            valid = False
            issue(stem, message, hard=True, metrics={"claim_id": claim_id, **metrics})
        if not isinstance(quote, str) or not quote.strip() or not any(quote in body for body in bodies):
            fail("PRODUCT_FACT_QUOTE_MISMATCH", "exact_copy_quote 必须是本组实际主评或单条回复的非空原文片段")
            continue
        normalized_quote = lh7_fact_text(quote)
        if not lh13_claim_matches(normalized_quote, claim):
            fail("PRODUCT_FACT_CLAIM_MISMATCH", "产品事实引用没有对应 claim 的可核验内容，不能只引用别处的型号")
        key = (claim_id, quote)
        if key in seen:
            fail("PRODUCT_FACT_DUPLICATE", "同一claim与原文引用重复登记")
        seen.add(key)
        model = normalize_model(record.get("target_model"))
        models = set(claim.get("models", []))
        if model == "unknown" or model not in models:
            fail("PRODUCT_FACT_MODEL_MISMATCH", "product_facts.target_model 必须明确且属于该claim的型号", target_model=model)
        quoted_models = extract_models(normalized_quote, lh7=True)
        if quoted_models - models or (quoted_models and model not in quoted_models):
            fail("PRODUCT_FACT_MODEL_MISMATCH", "原文中的型号与独立事实核验不一致", target_model=model, copy_models=sorted(quoted_models))
        if not quoted_models:
            # Reply pronouns may resolve to the actual parent, never a sibling
            # group. Allocation still separately verifies labels against post
            # or real personal evidence.
            group_models = extract_models(lh7_fact_text(group_copy(group)), lh7=True)
            if len(models) == 1 and group_models != {model}:
                fail("PRODUCT_FACT_MODEL_UNCLEAR", "单型号事实须在本组明确所指型号，不能用state替正文猜型号")
        sources = string_list(record.get("source_urls"))
        expected = {lh13_official_source_key(value) for value in claim.get("official_sources", [])}
        actual = {lh13_official_source_key(value) for value in (sources or [])}
        if not sources or "" in actual or not ((expected - {""}) & actual):
            fail("PRODUCT_FACT_SOURCE_MISMATCH", "事实来源须匹配该claim登记的DJI官方页面；帮助页必须匹配文章customId")
        checked_at = record.get("checked_at")
        try:
            checked = date.fromisoformat(checked_at)
            if checked > date.today() or (claim.get("fact_status_required") == "P2" and checked != date.today()):
                raise ValueError("stale_or_future")
        except (ValueError, TypeError):
            fail("PRODUCT_FACT_DATE_INVALID", "checked_at须为非未来日期，P2模式受限事实须当日核验", checked_at=checked_at)
        for terms in claim.get("required_text_groups", []):
            if not normalized_contains(normalized_quote, terms):
                fail("PRODUCT_FACT_BOUNDARY_MISSING", "独立事实原文缺少此claim不可省略的限定", missing_any_of=terms)
        for pattern in claim.get("forbidden_text_patterns", []):
            if re.search(pattern, normalized_quote, re.IGNORECASE):
                fail("PRODUCT_FACT_FORBIDDEN", "独立事实引用含已登记的禁写断言")
        if valid:
            valid_records.append((normalized_quote, claim, claim_id))
    uncovered = []
    for assertion in assertions:
        value = assertion.strip("，,。！？!?；;\n ")
        def covered_by(quote: str, claim: dict[str, Any]) -> bool:
            if value in quote:
                return lh13_claim_matches(value, claim)
            if quote in value and lh13_claim_matches(quote, claim):
                # The reviewer may quote the factual noun phrase alone, e.g.
                # “4p那个3倍镜头”; emotional prefixes are not required. Do not
                # let that fragment cover an additional hardware assertion.
                remainder = value.replace(quote, "", 1)
                return not LH13_HARDWARE_TERM_RE.search(remainder)
            return False
        if not any(covered_by(quote, claim) for quote, claim, _ in valid_records):
            uncovered.append(assertion)
    if uncovered and context.get("claim_mode") in {"observed", "personal_need", "question"}:
        issue("PRODUCT_FACT_UNREGISTERED", "独立产品断言缺少逐句有效product_facts，请登记事实、型号、官网原页与正文引用；不能改标question或P0绕过", hard=True, metrics={"assertions": uncovered})
    return uncovered, {claim_id for _, _, claim_id in valid_records}


def audit_lh13_seeding_review(sections: list[Section], contexts: dict[str, list[dict[str, Any]]],
                             state: dict[str, Any], diagnostics: list[dict]) -> dict[str, Any]:
    """Check reviewer evidence and byte binding, not the meaning of persuasion."""
    files = sorted({str(Path(section.file).resolve()) for section in sections})
    hashes_valid = True
    for filename in files:
        actual = hashlib.sha256(Path(filename).read_bytes()).hexdigest()
        expected = state.get("draft_sha256") if len(files) == 1 else (state.get("draft_hashes", {}).get(filename) if isinstance(state.get("draft_hashes"), dict) else None)
        if not isinstance(expected, str) or not re.fullmatch(r"[0-9a-f]{64}", expected) or expected != actual:
            hashes_valid = False
            diagnostics.append(diag("error", "E_DRAFT_HASH_MISMATCH", "LH13审稿记录必须绑定当前最终稿原始字节SHA256，修改后须重新审稿", filename, metrics={"actual": actual, "declared": expected}))
    qa = state.get("qa", {})
    semantic = qa.get("semantic_review", {}) if isinstance(qa, dict) else {}
    raw = semantic.get("seeding_review") if isinstance(semantic, dict) else None
    records: dict[str, list[dict]] = defaultdict(list)
    if not isinstance(raw, list):
        diagnostics.append(diag("error", "E_SEEDING_REVIEW_MISSING", "LH13须提供qa.semantic_review.seeding_review逐帖种草审稿记录"))
        raw = []
    for record in raw:
        if not isinstance(record, dict) or not isinstance(record.get("id"), str) or not record["id"].strip():
            diagnostics.append(diag("error", "E_SEEDING_REVIEW_INVALID", "seeding_review每项须为带非空allocation.id的对象"))
            continue
        records[record["id"]].append(record)
    counts = Counter(section.number for section in sections)
    used_ids: set[str] = set()
    passed = 0
    for section in sections:
        candidates = contexts.get(section.number, [])
        path = str(Path(section.file).resolve())
        matches = [candidate for candidate in candidates if candidate.get("_resolved_draft_path") in {None, "", path}] if counts[section.number] == 1 else [candidate for candidate in candidates if candidate.get("_resolved_draft_path") == path]
        allocation = matches[0] if len(matches) == 1 else {}
        allocation_id = str(allocation.get("id", ""))
        used_ids.add(allocation_id)
        current = records.get(allocation_id, [])
        valid = True
        def fail(code: str, message: str) -> None:
            nonlocal valid
            valid = False
            diagnostics.append(diag("error", code, message, section.file, section.line, section.number, metrics={"id": allocation_id}))
        if len(current) != 1:
            fail("E_SEEDING_REVIEW_MISSING" if not current else "E_SEEDING_REVIEW_DUPLICATE", "每个交付帖子须恰好一条种草审稿记录")
            continue
        record = current[0]
        if record.get("verdict") != "passed":
            fail("E_SEEDING_REVIEW_REVISE" if record.get("verdict") == "revise" else "E_SEEDING_REVIEW_INVALID", "种草语义审稿须完成并通过；revise不能交付")
        if not isinstance(record.get("reason"), str) or not keep_word_chars(record["reason"]):
            fail("E_SEEDING_REVIEW_REASON_MISSING", "须说明本帖如何产生具体Pocket兴趣，不能只有passed标签")
        entries = record.get("interest_entries")
        if not isinstance(entries, list) or not entries:
            fail("E_SEEDING_INTEREST_MISSING", "须引用至少一条主评中的具体Pocket兴趣，纯画面或只贴型号不能代替语义验收")
            continue
        seen_groups: set[int] = set()
        for entry in entries:
            if not isinstance(entry, dict) or type(entry.get("group")) is not int:
                fail("E_SEEDING_INTEREST_INVALID", "interest_entries每项须含实际group编号、正文引用和兴趣说明")
                continue
            number = entry["group"]
            group = next((g for g in section.groups if g.main.group == number), None)
            quote = entry.get("exact_copy_quote")
            interest = entry.get("interest")
            if group is None or number in seen_groups:
                fail("E_SEEDING_INTEREST_GROUP_INVALID", "兴趣引用须指向本帖实际主评且组号不重复")
            seen_groups.add(number)
            group_contexts = allocation.get("groups", [])
            mapped_contexts = [entry for entry in group_contexts if isinstance(entry, dict) and entry.get("group") == number] if isinstance(group_contexts, list) else []
            connection = mapped_contexts[0].get("product_connection") if len(mapped_contexts) == 1 else None
            if not isinstance(connection, str) or not connection.strip():
                fail("E_SEEDING_INTEREST_CONNECTION_MISSING", "种草引用组须记录实际产品关联；纯画面辅条不能独自充当本帖种草依据")
            if group is None or not isinstance(quote, str) or not quote.strip() or quote not in group.main.text:
                fail("E_SEEDING_INTEREST_QUOTE_MISMATCH", "兴趣引用必须是对应主评的实际非空原文，不能引用回复或历史稿")
            elif not keep_word_chars(LH7_PRODUCT_RE.sub("", quote)):
                fail("E_SEEDING_INTEREST_NAME_ONLY", "只引用产品名不能证明具体产品兴趣")
            if not isinstance(interest, str) or not keep_word_chars(interest):
                fail("E_SEEDING_INTEREST_INVALID", "须登记观看者从这条获得的具体Pocket兴趣")
        if valid and hashes_valid:
            passed += 1
    for orphan in records.keys() - used_ids:
        diagnostics.append(diag("error", "E_SEEDING_REVIEW_ORPHAN", "种草审稿记录未对应本次交付帖子", metrics={"id": orphan}))
    return {"sections_checked": len(sections), "records_passed": passed, "draft_hash_verified": hashes_valid,
            "note": "仅校验语义审稿记录和最终稿绑定，不能自动判断种草效果、活人感或引用事实为真"}


def lh13_user_material_error(section: Section, context: dict[str, Any], state: dict[str, Any]) -> str:
    """Bind link-free user input to one real local Markdown source section."""
    marker = re.fullmatch(r"user-material:([A-Za-z][A-Za-z0-9_-]{0,63})", section.url)
    capture = state.get("capture_policy", {})
    if not marker or not isinstance(capture, dict) or capture.get("primary_surface") != "user_supplied_evidence":
        return "user-material定位仅限明确提供用户材料的输入，不是平台链接读取替代入口"
    source_id = marker.group(1)
    if context.get("id") != source_id or context.get("canonical_url") != section.url:
        return "user-material的ID、allocation.id和canonical_url必须完全一致"
    blocked = {str(value) for field in ("failed_ids", "evidence_stop_ids") for value in (state.get(field, []) if isinstance(state.get(field), list) else [])}
    if {source_id, section.url} & blocked:
        return "失败或暂停的来源不能改为user-material定位绕过停项"
    source = context.get("user_material")
    if not isinstance(source, dict) or source.get("anchor") != source_id:
        return "user_material须提供本地path、同ID的Markdown标题anchor及exact_quote"
    if any(source.get(field) for field in ("source_url", "origin_url", "original_url", "remote_url")):
        return "已知平台来源使用原平台URL定位，不能通过user-material别名重映射远程来源"
    path = context.get("_resolved_user_material_path")
    quote = source.get("exact_quote")
    if not path or not isinstance(quote, str) or not quote.strip():
        return "user_material必须包含可读本地材料path及非空原文exact_quote"
    try:
        material_path = Path(path)
        if not material_path.is_file() or material_path.suffix.lower() not in {".md", ".txt"}:
            return "user_material.path须为实际存在的Markdown或文本材料文件"
        content = material_path.read_text(encoding="utf-8-sig")
    except (OSError, UnicodeError, ValueError):
        return "本地用户材料无法读取；不可用占位文件或未读来源完成验收"
    heading = re.search(r"(?m)^##[ \t]+" + re.escape(source_id) + r"[ \t]*$", content)
    if not heading:
        return "材料文件中没有对应的## ID标题锚点"
    end = re.search(r"(?m)^##[ \t]+", content[heading.end():])
    excerpt = content[heading.end():heading.end() + end.start()] if end else content[heading.end():]
    if quote not in excerpt or not keep_word_chars(quote) or quote.strip("` ") == section.url:
        return "user_material.exact_quote须来自该锚点下的真实材料，不能借其他帖或只填定位符"
    return ""
...
def lh10_group_product_facts(group: Group, context: dict[str, Any], *, lh11: bool = False, lh12: bool = False, lh13: bool = False) -> tuple[list[str], list[str]]:
    ...
    if lh13:
        implicit = re.compile(implicit.pattern + r"|\s*(?:按一下|单击|双击|长按|拍着拍着不用戳屏幕)")
    for item in [group.main, *group.replies]:
        ...
        for piece in re.finditer(r"[^，,。！？!?；;\n]+[，,。！？!?；;\n]?", lh7_fact_text(item.text)):
            ...
            assertion = (LH13_CAPABILITY_ASSERTION_RE if lh13 else LH8_CAPABILITY_ASSERTION_RE).search(clause)
            question = lh13_true_question(clause) if lh13 else bool(QUESTION_RE.search(clause))
            ...
            if refers_to_device and (LH13_HARDWARE_TERM_RE if lh13 else LH8_HARDWARE_TERM_RE).search(clause):
                if assertion and (lh13 or not match_is_negated(clause, assertion)) and not question:
                    capabilities.append(clause)
...
def lh11_visual_only_group(group: Group, context: dict[str, Any], *, lh12: bool = False, lh13: bool = False) -> bool:
    ...
    if any(lh7_product_matches(item.text) for item in [group.main, *group.replies]):
        return False
    capabilities, causes = lh10_group_product_facts(group, context, lh11=True, lh12=lh12, lh13=lh13)
...
def normalize_model(value: Any) -> str:
    if str(value or "").lower().strip() in {"初代", "一代", "pocket初代", "pocket一代"}:
        return "pocket_1"
    raw = re.sub(r"[^a-z0-9]", "", str(value or "").lower())
    raw = re.sub(r"^dji", "", raw)
    aliases = {
        "pocket1": "pocket_1",
...
def audit_seed_semantics(
    ...
    *, personal_need_groups: set[tuple[str, str, int]] | None = None,
    ...
    """Audit LH5/LH6/LH7/LH8/LH9/LH10/LH11/LH12/LH13 evidence contracts without pretending regex can assign S grades.
    ...
    if not state:
        add_seed_diag(
            ...
            policy,
            "SEED_STATE_INVALID",
            "--state 文件为空或缺少 LH5/LH6/LH7/LH8/LH9/LH10/LH11/LH12/LH13 顶层结构",
            hard=True,
        )
        return {
    ...
    lh12 = rule_version in LH12_PLUS_RULE_VERSIONS
    lh13 = rule_version == LH13_RULE_VERSION
    ...
    if rule_version not in SUPPORTED_RULE_VERSIONS:
        add_seed_diag(
            ...
            policy,
            "SEED_RULE_VERSION",
            "state.rule_version 必须为受支持的 LH5–LH13 版本",
    ...
    if not isinstance(semantic_policy, dict) or any(
        ...
        add_seed_diag(
            ...
            "state.semantic_policy 与 LH5/LH6/LH7/LH8/LH9/LH10/LH11/LH12/LH13 检查协议不一致",
            file=state.get("output_path") or None,
    ...
    if "evidence_stop_ids" not in state:
        add_seed_diag(
            ...
            policy,
            "SEED_STATE_INCOMPLETE",
            "LH5/LH6/LH7/LH8/LH9/LH10/LH11/LH12/LH13 state 缺少 evidence_stop_ids",
            file=state.get("output_path") or None,
        )
    ...
    for section in sections:
        ...
        if not candidates:
            ...
            add_seed_diag(
                ...
                f"章节 {section.number} 缺少 LH5/LH6/LH7/LH8/LH9/LH10/LH11/LH12/LH13 证据映射",
                section,
            )
            continue
        ...
        user_material_input = lh13 and str(section.url).startswith("user-material:")
        if user_material_input:
            source_error = lh13_user_material_error(section, context, state)
            if source_error:
                missing_context_sections += 1
                add_seed_diag(diagnostics, policy, "USER_MATERIAL_INVALID", source_error, section, hard=True)
                continue
        if not user_material_input and (not actual_canonical_url or actual_platform == "unknown"):
            missing_context_sections += 1
            add_seed_diag(
                diagnostics,
                policy,
            ...
            continue
        if not user_material_input and (not context.get("canonical_url") or not state_canonical_url or state_platform == "unknown"):
            missing_context_sections += 1
            ...
            continue
        if not user_material_input and (state_canonical_url != actual_canonical_url or state_platform != actual_platform):
            missing_context_sections += 1
            add_seed_diag(
                diagnostics,
                policy,
        ...
        if missing:
            ...
            add_seed_diag(
                ...
                f"章节 {section.number} 的 LH5/LH6/LH7/LH8/LH9/LH10/LH11/LH12/LH13 证据字段不完整",
                section,
        ...
        source_key = lh13_official_source_key if lh13 else normalized_source_url
        ...
        supplied_source_keys = {
            source_key(source) for source in source_urls if source_key(source)
        }
        ...
        if model_evidence_type not in VALID_MODEL_EVIDENCE_TYPES:
            add_seed_diag(
                ...
                "MODEL_EVIDENCE_TYPE_INVALID",
                    "model_status.evidence_type 不是 LH5/LH6/LH7/LH8/LH9/LH10/LH11/LH12/LH13 允许值",
                section,
        ...
        for group in section.groups:
            ...
            if actual_s_missing:
                group_issue(
                    ...
                    "LH5/LH6/LH7/LH8/LH9/LH10/LH11/LH12/LH13 严格检查要求显式填写 actual_s_level",
            ...
            lh10_capabilities, lh10_causes = lh10_group_product_facts(group, group_context, lh11=lh11, lh12=lh12, lh13=lh13) if lh10 else ([], [])
            independent_fact_ids: set[str] = set()
            if lh13:
                lh10_capabilities, independent_fact_ids = audit_lh13_product_facts(
                    group, group_context, claims, lh10_capabilities, group_issue)
            ...
            if (lh8 and mode == "personal_need") or (lh10 and mode == "observed"):
                ...
                if capability_claims:
                    group_issue(mode_code + "_PRODUCT_ASSERTION", (f"{mode} 中的独立官网事实尚未通过 product_facts 核验，请补齐本句事实记录" if lh13 else f"{mode} 不能代替硬件功能核验，含产品能力断言时应改用对应的产品证据模式"), hard=True, metrics={"claims": capability_claims})
                if lh10:
            ...
            for registered_claim_id, registered_claim in claims.items():
                ...
                for pattern in raw_patterns:
                    try:
                        ...
                        if lh13:
                            # A real question about an unsupported feature is
                            # not an assertion that it exists (or does not).
                            matches = list(re.finditer(str(pattern), text, re.IGNORECASE))
                            match = None
                            for candidate in matches:
                                start = max(text.rfind(mark, 0, candidate.start()) for mark in "，,。！？!?；;\n") + 1
                                end = next((index for index in range(candidate.end(), len(text)) if text[index] in "，,。！？!?；;\n"), len(text))
                                if not lh13_true_question(text[start:end+1]) and not match_is_negated(text, candidate):
                                    match = candidate
                                    break
                    except re.error:
                        continue
            ...
            for registered_claim_id, registered_claim in claims.items():
                ...
                if (
                    ...
                    and registered_claim_id not in (verified_claim_ids | independent_fact_ids)
                    and (not lh13 or any(not lh13_true_question(clause) and normalized_contains(clause, registered_text_terms) for clause in lh10_capabilities))
                ):
                    group_issue(
            ...
            if lh10 and mode == "observed" and target_s == actual_s == "S0" and group_valid:
                ...
                if (claim_id_value == "" and benefit_basis in {"workflow", "visual"}
                        ...
                        and ((isinstance(connection, str) and connection.strip())
                             or (lh11 and lh11_visual_only_group(group, group_context, lh12=lh12, lh13=lh13)))
...
def audit(
    ...
    *,
    ...
    seeding_review_stats = audit_lh13_seeding_review(sections, contexts, state, diagnostics) if state.get("rule_version") == LH13_RULE_VERSION else None
    ...
    if seeding_policy == "off":
        seed_stats: dict[str, Any] = {"status": "off", "note": "LH5/LH6/LH7/LH8/LH9/LH10/LH11/LH12/LH13 seeding checks disabled"}
    else:
        personal_need_groups: set[tuple[str, str, int]] = set()
        observed_s0_groups: set[tuple[str, str, int]] = set()
        seed_stats = {}
        registry = load_claim_registry(claim_registry_path or DEFAULT_CLAIM_REGISTRY, seeding_policy, diagnostics, lh13=state.get("rule_version") == LH13_RULE_VERSION)
    ...
    return {
        ...
        **({"seeding_review": seeding_review_stats} if seeding_review_stats is not None else {}),
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default="off",
        help="Legacy two-clear-one-hidden policy; use off for LH5/LH6; ignored by LH7/LH8/LH9/LH10/LH11/LH12/LH13",
    )
    parser.add_argument("--state", type=Path, help="LH5/LH6/LH7/LH8/LH9/LH10/LH11/LH12/LH13 pocket_comment_state.json with per-section M/N/F/P evidence")
    parser.add_argument(
        "--seeding-policy",
        choices=("advisory", "strict", "off"),
        default="advisory",
        help="How to enforce LH5/LH6/LH7/LH8/LH9/LH10/LH11/LH12/LH13 evidence-aware product seeding checks",
    )
    parser.add_argument(
        "--claim-registry",
        type=Path,
